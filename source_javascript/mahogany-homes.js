...
var createUi = state => {
  ...
  var mainPanel = createMainPanel(colorScheme, 'Contract Selection');
  var contractDropdown = createDropdown('Contract Type', 'contractType', ['Beginner', 'Novice', 'Intermediate', 'Expert'], 'Beginner', colorScheme.TEXT);
...
var stateManager = () => {
  logger(state, 'debug', "stateManager: ".concat(state.main_state, ".").concat(state.sub_state), "Function start.");
  switch (state.main_state) {
    case 'walk_to_amy':
      {
        if (!bot.localPlayerIdle() || bot.walking.isWebWalking()) break;
        var isPlayerAtStartLocation = () => locationFunctions.isPlayerNearWorldPoint(scriptLocations.startLocation) && npcFunctions.npcExists(npcIds.falador.amy);
        if (!isPlayerAtStartLocation()) {
          logger(state, 'all', "stateManager: ".concat(state.main_state, ".").concat(state.sub_state), 'Walking to Amy in Falador.');
          ...
          timeoutManager.add({
            ...
            maxWait: 200,
            onFail: () => {
              throw new Error('Unable to locate Amy in Falador after 200 ticks.');
            }
          });
          break;
        }
        logger(state, 'debug', "stateManager: ".concat(state.main_state, ".").concat(state.sub_state), 'Player is at start location.');
    ...
    case 'get_contract':
      {
        switch (state.sub_state) {
          case 'default':
            {
              var isContractSelectVisible = () => widgetFunctions.widgetExists(widgetData.dialogue.mahogany_homes.amy.select_contract.identifier);
              if (!isContractSelectVisible()) {
                logger(state, 'debug', "stateManager: ".concat(state.main_state, ".").concat(state.sub_state), "Getting a {state.contractType} contract from Amy.");
              }
            }
        }
      }
